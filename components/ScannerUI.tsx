...
export const ScannerUI: React.FC<ScannerUIProps> = ({ onResults, onLoading }) => {
  ...
  const initiateSession = () => {
    ...
    try {
      const MERCHANT_KEY = import.meta.env.VITE_BANI_KEY; // pulled from .env file
      ...
      BaniPopUp({
        ...
        metadata: { tier: activeTier, custom_ref: `${sessionId}-${activeTier}-${Date.now()}` },
        callback: (response) => {
          setIsProcessing(false);
          if (response.status === 'success') {
            const licenseKey = `OM-REF-${activeTier}-${response.metadata?.custom_ref || sessionId}`;
            ...
            addLog('Payment flow closed or cancelled by user.', 'warning');
          }
        }
      });
    } catch (err: any) {
  ...
  return (
    <section className="w-full max-w-6xl mx-auto p-6 space-y-16 relative pb-20">
